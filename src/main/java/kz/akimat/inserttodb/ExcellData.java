...
public class ExcellData {
    ...
    public ExcellData(Row row, String districtId) {
        this.protocolNumber = districtId + "-" + getStringFromRowByIndex(row.getCell(10));//Номер протокола
        ...
        String statusText = row.getCell(5).getStringCellValue();//Статус
        ...
        this.result = row.getCell(6).getStringCellValue();//Информация по исполнению
        this.sphere = getStringFromRowByIndex(row.getCell(7));//Сфера
        this.protocolTitle = getStringFromRowByIndex(row.getCell(8));//Наименование протокола
        this.protocolDate = getDateFromRowByIndex(row.getCell(9));//Дата протокола
        this.protocolType = getStringFromRowByIndex(row.getCell(11));//Тип протокола
